<ul>
    <li><b><?php echo anchor("admin/categories/","Manage Categories");?>.</b>
        <br/>
        Create, edit, delete and manage product categories on your online store.
    </li>
    <br/>
    <li><b><?php echo anchor("admin/products/","Manage Products");?>.</b>
        <br/>
        Create, edit, delete and manage products on your online store.
    </li>

    <br/>

    <li><b><?php echo anchor("admin/admins/","Manage Users");?>.</b>
        <br/>
        Create, edit, delete and manage users who can access this dashboard.
    </li>
    <br/>

    <li><b><?php echo anchor("admin/colors/","Manage Colors");?>.</b>
        <br/>
        Create, edit, delete and manage colors available to products .
    </li>
    <br/>

    <li><b><?php echo anchor("admin/sizes/","Manage Sizes");?>.</b>
        <br/>
        Create, edit, delete and manage size available to products.
    </li>
    <br/>

    <li><b><?php echo anchor("admin/pages/","Manage Pages");?>.</b>
        <br/>
        Create, edit, delete and manage pages in your online store.
    </li>
    <br/>

    <li><b><?php echo anchor("admin/subscribers/","Manage Subscribers");?>.</b>
        <br/>
        Manage subscribers and send out email .
    </li>
    <br/>

    <li><b><?php echo anchor("admin/dashboard/logout/","Logout");?>.</b>
        <br/>
        Exit this dashboard when you're done.
    </li>
    <br/>
</ul>